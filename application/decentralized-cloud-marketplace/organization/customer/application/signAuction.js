...
async function main() {
    ...
    try {
        ...
        await contract.addContractListener('StartProvisioningEvent', (err, event, blockNumber, transactionId, status) => {
            if (err) {
                console.error(err);
                return;
            }
            let _event = JSON.parse(event);
            console.log(event);
            console.log(_event);
            console.log(`Block Number: ${blockNumber} Transaction ID: ${transactionId} Status: ${status}`);
            setTimeout(() => console.log("Done Provisioning"), 5000);
        })
